...
export function HabitsTab() {
  ...
  const handleToggleCompletion = async (habit: Habit, dateStr: string, isCompleted: boolean) => {
    ...
    await db.transaction('rw', db.habits, db.habit_completions, db.sync_deletions, async () => {
      ...
      // Recalculate streak
      const allComps = await db.habit_completions.where('habit_id').equals(habit.id).filter(x => x.user_id === (useAppStore.getState().userId || 'default')).toArray();
      ...
      if (completedDates.size === 0) {
        await db.habits.update(habit.id, { streak_current: 0 });
        return;
      }
      ...
      let currDate = new Date(minDateStr + 'T12:00:00Z');
      const endDate = new Date(todayStr + 'T12:00:00Z');

      // To prevent scanning infinite histories, bound the minDate to a reasonable past if necessary, 
      // but users generally don't have > 10,000 days of habits so this loop is instant.
      ...
      while (currDate <= endDate) {
        ...
        if (isActiveDay) {
          if (completedDates.has(iterDateStr)) {
            ...
            // Break streak if we missed an active day, EXCEPT if that day is today (we still have time)
